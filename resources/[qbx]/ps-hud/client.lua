...
RegisterNetEvent("hud:client:LoadMap", function()
    ...
    -- Determine which map shape to use
    local mapShape = Menu.isToggleMapShapeChecked == "square" and "squaremap" or "circlemap"
    ...
    -- Adjust positioning based on map shape
    if Menu.isToggleMapShapeChecked == "square" then
        -- Square minimap positioning
        SetMinimapComponentPosition("minimap", "L", "B", -0.0085 + minimapOffset, -0.025, 0.150, 0.188)
        SetMinimapComponentPosition("minimap_mask", "L", "B", 0.200 + minimapOffset, 0.0, 0.065, 0.20)
        SetMinimapComponentPosition('minimap_blur', 'L', 'B', -0.01 + minimapOffset, 0.025, 0.262, 0.300)
    else
        -- Circle minimap positioning (original)
...
local function getFuelLevel(vehicle)
    ...
    if (updateTick - lastFuelUpdate) > 2000 then
        ...
        -- Check if using ox_fuel
        if Config.FuelScript == 'ox_fuel' then
            local vehState = Entity(vehicle).state
            if vehState.fuel then
                lastFuelCheck = math.floor(vehState.fuel)
            else
                -- Fallback to native fuel level if state not available
                lastFuelCheck = math.floor(GetVehicleFuelLevel(vehicle))
            end
        else
            -- For other fuel systems, use the export
    ...
    while true do
        if LocalPlayer.state.isLoggedIn then
            ...
            if IsPedInAnyVehicle(ped, false) and not IsThisModelABicycle(GetEntityModel(GetVehiclePedIsIn(ped, false))) and not isElectric(GetVehiclePedIsIn(ped, false)) then
                if getFuelLevel(GetVehiclePedIsIn(ped, false)) <= 20 then -- At 20% Fuel Left
